        ...
        {options.map((opt) => {
          ...
          return (
            <button
              key={opt}
              onClick={() => setCat(opt)}
              className={cx("rounded-2xl px-3 py-3 text-left transition-colors", active ? "text-white" : "text-white/60 hover:text-white/80")}
            >
              ...
              <div className={cx("mt-1 text-[11px]", active ? "text-white/60" : "text-white/40")}>{laneSub(opt)}</div>
            </button>
          );
        })}
      </div>
...
function CollageCard({
  ...
  return (
    <motion.article
      className={cx("group relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03]", "lg:col-span-12")}
      style={{
