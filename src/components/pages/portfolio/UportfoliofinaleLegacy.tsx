...
/* =======================================================================================
   Wodh — Portfolio Hybrid Page (FULL REWRITE w/ Patch Set v1 applied)

   Structure:
   1) Hero
   2) Studio selector strip (All + XR + Games + 3D) + Search
   3) Featured 4 (2×2 big cards)
   4) Outcomes editorial band
   5) Collage grid (tetris spans) — full library
   6) Mini pipeline (true connected rail)
   7) Proof wall (logo marks marquee + quotes)
   8) Case study teasers (3)
   9) Final CTA

   Color lanes:
   - XR    = neon green (colors.neon.base)
   - Games = violet/indigo (colors.violet.base)
...
export default function UportfoliofinaleLegacy() {
  const reduceMotion = useReducedMotion() ?? false;
  const [cat, setCat] = useState<Category>("All");
  const [query, setQuery] = useState("");

  const counts = useMemo(() => {
    const by: Record<Category, number> = { All: PROJECTS.length, XR: 0, Games: 0, "3D": 0 };
  ...
  const featured4 = useMemo(() => {
    ...
    const feat = pool.filter((p) => p.featured).sort((a, b) => b.year - a.year);
    const non = pool.filter((p) => !p.featured).sort((a, b) => b.year - a.year);
    return [...feat, ...non].slice(0, 4);
  }, [cat]);
  ...
  return (
    ...
    >
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-6 shadow-[0_30px_120px_rgba(0,0,0,0.55)] sm:p-10">
          ...
          <div className="relative flex flex-col gap-7">
            <div className="flex flex-wrap items-center gap-2">
              <Chip>Portfolio</Chip>
              <Chip variant="default">XR • Games • 3D</Chip>
              <Chip variant="default">Curated proof</Chip>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <p className="max-w-3xl text-pretty text-sm leading-relaxed sm:text-base" style={blendTextStyle()}>
              ...
              <p className="max-w-3xl text-sm leading-relaxed text-white/55 sm:text-base">
                Three lanes, one world: <span style={{ color: TOKENS.xr }}>XR</span>,{" "}
                <span style={{ color: TOKENS.games }}>Games</span>,{" "}
                <span style={{ color: TOKENS.d3 }}>3D</span>.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <StatPill label="Mode" value={laneName(cat)} />
      ...
      <section id="portfolio-grid" className="relative mx-auto w-full max-w-6xl px-4 pt-10 sm:px-6 sm:pt-12">
        ...
        <div className="mt-6">
          <CollageGrid items={filtered.filter((p) => !p.featured)} />
        </div>
...
function SectionHeader({ chip, title, lead }: { chip: string; title: string; lead: string }) {
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        <Chip>{chip}</Chip>
        <Chip variant="default">Portfolio</Chip>
      </div>
      <h2 className="text-balance text-2xl font-semibold tracking-[-0.03em] text-white sm:text-3xl">{title}</h2>
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-2">
      ...
      <motion.div
        ...
        style={{ width: `calc((100% - 24px) / 4)`, background: laneUnderline(cat) }}
        animate={{ x: `calc(${activeIndex} * ((100% - 24px) / 4))` }}
        transition={{ duration: reduceMotion ? 0 : duration.fast, ease: [0.2, 0.8, 0.2, 1] }}
      />
      ...
      <div className="relative grid grid-cols-4 gap-2">
        {options.map((opt) => {
          ...
          const dot =
            opt === "XR" ? TOKENS.xr : opt === "Games" ? TOKENS.games : opt === "3D" ? TOKENS.d3 : "rgba(255,255,255,0.40)";

          return (
            <button
              key={opt}
              onClick={() => setCat(opt)}
              className={cn("rounded-2xl px-3 py-3 text-left transition-colors", active ? "text-white" : "text-white/60 hover:text-white/80")}
            >
              ...
              <div className={cn("mt-1 text-[11px]", active ? "text-white/60" : "text-white/40")}>{laneSub(opt)}</div>
            </button>
          );
        })}
      </div>
...
function FeaturedCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <motion.article
      ...
      style={{ boxShadow: `0 30px 120px rgba(0,0,0,0.55), inset 0 0 0 1px rgba(255,255,255,0.04)` }}
    >
      <div className="relative h-[260px] overflow-hidden sm:h-[320px]">
        {project.video ? (
          <video className="absolute inset-0 h-full w-full object-cover opacity-90" src={project.video} autoPlay loop muted playsInline />
        ) : (
          <motion.img
            src={project.thumb}
            alt={project.title}
        ...
        {/* Patch v1 — tinted overlay */}
        <div className="absolute inset-0" style={{ background: cardTintOverlay(a) }} />
        ...
        <div className="absolute left-5 top-5 flex flex-wrap items-center gap-2">
          ...
          <Chip variant="default">{project.year}</Chip>
          {project.caseStudy ? <Chip variant="default">Case</Chip> : <Chip variant="default">Snapshot</Chip>}
        </div>

        <div className="absolute bottom-5 left-5 right-5">
          <div className="text-[11px] font-semibold text-white/60">FEATURED {index + 1}</div>
          <div className="mt-1 text-lg font-semibold tracking-[-0.02em] text-white sm:text-xl">{project.title}</div>
      ...
      <div className="flex items-center justify-between gap-3 p-5">
        <div className="text-[11px] text-white/50">{project.caseStudy ? "Open case study" : "Details on request"}</div>
        <button
          onClick={() => {
          ...
          className={cn(
            ...
            project.caseStudy ? "border-white/10 bg-white/[0.03] text-white/80 hover:bg-white/[0.06]" : "border-white/10 bg-white/[0.02] text-white/55"
          )}
        >
...
/* =======================================================================================
   Outcomes (Patch v1 — editorial band)
======================================================================================= */
  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
      {grid.map(({ it, span }, idx) => (
        <CollageCard key={it.id} project={it} colSpan={span.c} rowSpan={span.r} index={idx} reduceMotion={reduceMotion} />
      ))}
    </div>
  );
}
...
function CollageCard({
  ...
  return (
    <motion.article
      className={cn("group relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03]", "lg:col-span-12")}
      style={{
    ...
    >
      <div className="relative h-full min-h-[220px]">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <div className="text-sm font-semibold tracking-[-0.02em] text-white sm:text-base">{project.title}</div>
          <div className="mt-1 line-clamp-2 text-xs text-white/70 sm:text-sm">{project.subtitle}</div>
        ...
        <button
          ...
          className={cn(
            ...
            project.caseStudy ? "border-white/10 bg-white/[0.03] text-white/75 hover:bg-white/[0.06]" : "border-white/10 bg-white/[0.02] text-white/55"
          )}
...
/* =======================================================================================
   Mini pipeline (Patch v1 — true connected rail)
======================================================================================= */
    ...
    <div className="mt-6 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03]">
      ...
      <div className="relative p-6 sm:p-8">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(90deg, rgba(158,243,21,0.55), rgba(255,255,255,0.10) 45%, rgba(91,45,220,0.55))",
          }}
        />
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-5">
          {steps.map((s) => (
            <div key={s.title} className="relative rounded-3xl border border-white/10 bg-white/[0.02] p-5">
              ...
              <div aria-hidden className="pointer-events-none absolute inset-0 rounded-3xl" style={{ boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.03)" }} />
            </div>
          ))}
        </div>
      </div>
...
/* =======================================================================================
   Proof wall (Patch v1 — SVG marks marquee)
======================================================================================= */
...
function ProofWall({ quotes }: { quotes: Quote[] }) {
  ...
  const marks = [
    ...
    { name: "Unity", svg: <LogoMark word="UNITY" /> },
    { name: "Unreal", svg: <LogoMark word="UNREAL" /> },
    { name: "Vision Pro", svg: <LogoMark word="VISION" /> },
    { name: "Steam", svg: <LogoMark word="STEAM" /> },
    { name: "WebXR", svg: <LogoMark word="WEBXR" /> },
    { name: "Console", svg: <LogoMark word="CONSOLE" /> },
    { name: "Enterprise", svg: <LogoMark word="ENTERPRISE" /> },
  ];
      ...
      <div className="grid gap-4 lg:col-span-5">
        {quotes.slice(0, 2).map((q) => (
          <div key={q.text} className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
            <div className="text-sm leading-relaxed" style={blendTextStyle()}>
              “{q.text}”
            </div>
...
function LogoMark({ word }: { word: string }) {
  return (
      ...
      {items.map((d) => {
        ...
        return (
          <motion.article
            ...
            whileHover={reduceMotion ? undefined : { y: -2 }}
            transition={{ duration: duration.fast, ease: easing.smooth }}
            style={{ boxShadow: `0 26px 110px rgba(0,0,0,0.40), inset 0 0 0 1px rgba(255,255,255,0.04)` }}
          >
            <div
              aria-hidden
            ...
            <div className="relative flex flex-col gap-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {d.caseStudy ? <Chip variant="default">Case study</Chip> : <Chip variant="default">NDA</Chip>}
              </div>
              ...
              <div className="flex items-center justify-between pt-2 text-[11px] text-white/50">
                ...
                <button
                  ...
                  className={cn(
                    "rounded-2xl border px-3 py-2 text-xs font-semibold",
                    d.caseStudy ? "border-white/10 bg-white/[0.03] text-white/80 hover:bg-white/[0.06]" : "border-white/10 bg-white/[0.02] text-white/55"
                  )}
                >
    ...
    <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-7 sm:p-10">
      ...
      <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="max-w-xl">
          <div className="flex flex-wrap items-center gap-2">
            <Chip>Next</Chip>
            <Chip variant="default">One team. Three lanes.</Chip>
...
function blendTextStyle(): React.CSSProperties {
  return {
    background: "linear-gradient(90deg, rgba(158,243,21,0.92), rgba(255,255,255,0.62) 40%, rgba(91,45,220,0.92))",
    WebkitBackgroundClip: "text",
    backgroundClip: "text",
    color: "transparent",
  };
