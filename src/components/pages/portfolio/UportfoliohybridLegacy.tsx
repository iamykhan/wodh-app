...
/* =======================================================================================
   ...
   - Category accents:
       ...
       Games = violet/indigo (colors.violet.base)  (still distinct from base)
...
export default function UportfoliohybridLegacy() {
...
function SegmentedCategory({
  ...
  const activeIndex = options.indexOf(cat);

  return (
    <div className="relative w-full rounded-3xl border border-white/10 bg-white/[0.03] p-1 sm:p-1.5">
      <motion.div
        className="absolute top-1.5 h-[calc(100%-12px)] rounded-2xl"
        style={{
          width: `calc((100% - 12px) / 4)`,
          left: `calc(${activeIndex} * ((100% - 12px) / 4) + 6px)`,
          background: "rgba(255,255,255,0.06)",
          boxShadow:
            cat === "XR"
              ? "0 0 0 1px rgba(255,255,255,0.08), 0 0 28px rgba(158,243,21,0.20)"
              : cat === "Games"
                ? "0 0 0 1px rgba(255,255,255,0.08), 0 0 28px rgba(91,45,220,0.20)"
                : cat === "3D"
                  ? "0 0 0 1px rgba(255,255,255,0.08), 0 0 28px rgba(246,183,74,0.20)"
        ...
        {options.map((opt) => (
          <button
            ...
            className={cn(
              ...
              opt === cat ? "text-white" : "text-white/60 hover:text-white/80"
            )}
          >
                  ...
                  background:
                    opt === "XR"
                      ...
                      : opt === "Games"
                        ? "rgba(91,45,220,0.95)"
                        : opt === "3D"
                          ? "rgba(246,183,74,0.95)"
                          : "rgba(255,255,255,0.40)",
                  boxShadow:
                    opt === "XR"
                      ? "0 0 14px rgba(158,243,21,0.18)"
                      : opt === "Games"
                        ? "0 0 14px rgba(91,45,220,0.18)"
                        : opt === "3D"
                          ? "0 0 14px rgba(246,183,74,0.18)"
                          : "none",
                }}
              />
              <span>{labelFor(opt)}</span>
              <span className="text-[11px] font-semibold text-white/45">{counts[opt]}</span>
            </div>
          </button>
        ))}
      </div>
...
function OutcomesStrip({ cat }: { cat: Category }) {
  const items = useMemo(() => {
    // keep outcomes “universal” but slightly biased by category
    if (cat === "XR") {
      return ["Faster onboarding", "Safer operations", "Repeatable training", "Stakeholder clarity", "Premium demos", "Lower training cost"];
    }
    if (cat === "Games") {
      return ["Stable sessions", "Better retention", "Clean UX flow", "Performance budgets", "Live tuning loops", "Scalable systems"];
    }
    if (cat === "3D") {
      return ["Style consistency", "Clean lookdev", "Optimized assets", "Faster reviews", "Premium renders", "Reusable kits"];
    }
    return ["Clarity-first delivery", "Production readiness", "Performance budgets", "Premium visuals", "Reliable systems", "Fast iteration"];
  }, [cat]);

  const accent =
    cat === "All" ? "linear-gradient(90deg, rgba(158,243,21,0.18), rgba(255,255,255,0.05) 45%, rgba(91,45,220,0.18))"
      : `linear-gradient(90deg, ${rgba(accentFor(cat as any), 0.18)}, rgba(255,255,255,0.06) 55%, ${rgba(accentFor(cat as any), 0.12)})`;
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0" style={{ background: accent }} />
      ...
      <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          {items.map((x) => (
            <span
              key={x}
              className="rounded-full border border-white/10 bg-white/[0.02] px-3 py-1 text-[11px] font-semibold text-white/70"
            >
              {x}
            </span>
          ))}
...
function MiniPipeline() {
  const steps = [
    ...
    { title: "Production", desc: "Build systems, content, and pipelines." },
  ...
  return (
    <div className="mt-6 rounded-3xl border border-white/10 bg-white/[0.03] p-6 sm:p-8">
      <div className="relative">
        {/* connected track */}
        <div className="pointer-events-none absolute left-0 right-0 top-[22px] hidden h-px bg-white/10 sm:block" />
        <div
          className="pointer-events-none absolute left-0 right-0 top-[22px] hidden h-px opacity-70 sm:block"
          style={{
            background:
              "linear-gradient(90deg, rgba(158,243,21,0.55), rgba(255,255,255,0.14) 45%, rgba(91,45,220,0.55))",
          }}
        />

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-5">
          {steps.map((s) => (
            <div key={s.title} className="rounded-3xl border border-white/10 bg-white/[0.02] p-5">
...
function ProofWall({ quotes, reduceMotion }: { quotes: Quote[]; reduceMotion: boolean }) {
  // “logos” as simple placeholders — swap with real SVG later
  const logos = ["Meta", "Unity", "Unreal", "Vision Pro", "Steam", "WebXR", "Console", "Enterprise"];
  ...
  return (
    <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-12">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-6 lg:col-span-7">
        ...
        <div className="mt-1 text-sm text-white/55">A calm trust layer — not a loud wall.</div>
        ...
        <div className="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
          ...
          >
            {[...logos, ...logos].map((l, i) => (
              <div
                key={`${l}-${i}`}
                className="flex h-11 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.02] px-4 text-xs font-semibold text-white/65"
              >
                {l}
              </div>
            ))}
...
function blendTextStyle(): React.CSSProperties {
  return {
    background:
      "linear-gradient(90deg, rgba(158,243,21,0.95), rgba(255,255,255,0.72) 44%, rgba(91,45,220,0.92))",
    WebkitBackgroundClip: "text",
    backgroundClip: "text",
    color: "transparent",
  };
